...
public class MainActivity extends Activity {
    ...
    private void injectNativeProjects(WebView view){
        String js="javascript:(()=>{const n=typeof AxilortStorage!=='undefined';if(!n)return;const area=document.getElementById('projectArea');const btn=document.getElementById('newProject');if(!area||!btn)return;let render=()=>{let a=JSON.parse(AxilortStorage.list('projects')||'[]').filter(x=>x.folder);area.innerHTML='';if(!a.length){area.innerHTML='<div style=\"padding:50px;text-align:center;color:#777\">No projects yet<br><small>Create your first Axilort project.</small></div>';return}a.forEach(x=>{let d=document.createElement('div');d.style='position:relative;min-height:58px;display:flex;align-items:center;gap:12px;padding:10px;margin:7px 0;border:1px solid #242426;border-radius:12px;background:#0b0b0c';d.innerHTML='<div style=\"font-size:22px;color:#ff2028\">□</div><div style=\"flex:1;min-width:0\"><b style=\"display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis\">'+x.name.replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',\"'\":'&#39;'}[c]))+'</b><small style=\"color:#777\">axilort-engine-data/projects/'+x.name+'</small></div><button style=\"background:#111;color:#fff;border:1px solid #333;border-radius:7px;padding:7px 10px\">⋮</button>';d.querySelector('button').onclick=()=>{let act=prompt('Type: open, rename, or delete');if(act==='open'){location.href='assets.html'}else if(act==='rename'){let nn=prompt('New project name',x.name);if(nn&&AxilortStorage.rename('projects/'+x.name,'projects/'+nn))render()}else if(act==='delete'){if(confirm('Delete '+x.name+' and all its files?')){AxilortStorage.delete('projects/'+x.name);render()}}};area.appendChild(d)})};btn.onclick=()=>{let name=prompt('Project name');if(name&&AxilortStorage.createProject(name)){render()}};render();})();";view.evaluateJavascript(js,null);
    }
    ...
    private File safeFile(String relative)throws Exception{File root=storageRoot().getCanonicalFile();File file=new File(root,relative==null?"":relative.replace('\\','/')).getCanonicalFile();String rp=root.getPath();if(!file.getPath().equals(rp)&&!file.getPath().startsWith(rp+File.separator))throw new SecurityException("Invalid path");return file;}

    public class AxilortStorage {
        @JavascriptInterface public String rootPath(){return storageRoot().getAbsolutePath();}
        @JavascriptInterface public boolean createProject(String name){try{if(name==null)return false;name=name.trim();if(name.isEmpty()||name.contains("/")||name.contains("\\"))return false;File p=safeFile("projects/"+name);if(p.exists())return false;if(!p.mkdirs())return false;new File(p,"assets").mkdirs();new File(p,"scenes").mkdirs();new File(p,"scripts").mkdirs();new File(p,"project.json").createNewFile();writeText("projects/"+name+"/project.json","{\n  \"name\": \""+name.replace("\\","\\\\").replace("\"","\\\"")+"\",\n  \"engine\": \"Axilort Engine\",\n  \"version\": \"1.0.0\"\n}\n");return true;}catch(Exception e){return false;}}
        @JavascriptInterface public String list(String relative){try{File dir=safeFile(relative);if(!dir.exists())dir.mkdirs();JSONArray out=new JSONArray();File[] fs=dir.listFiles();if(fs==null)return out.toString();for(File f:fs){JSONObject o=new JSONObject();o.put("name",f.getName());o.put("path",relative==null||relative.isEmpty()?f.getName():relative+"/"+f.getName());o.put("folder",f.isDirectory());o.put("size",f.isFile()?f.length():0);o.put("modified",f.lastModified());out.put(o);}return out.toString();}catch(Exception e){return "[]";}}
        ...
        @JavascriptInterface public boolean rename(String from,String to){try{File a=safeFile(from),b=safeFile(to);File p=b.getParentFile();if(p!=null)p.mkdirs();return a.renameTo(b);}catch(Exception e){return false;}}
        ...
        @JavascriptInterface public boolean writeText(String relative,String text){try{File f=safeFile(relative);File p=f.getParentFile();if(p!=null)p.mkdirs();try(FileOutputStream out=new FileOutputStream(f)){out.write(text.getBytes(StandardCharsets.UTF_8));}return true;}catch(Exception e){return false;}}
