...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        s.setJavaScriptEnabled(true); s.setDomStorageEnabled(true); s.setDatabaseEnabled(false); s.setAllowFileAccess(true); s.setAllowContentAccess(true); s.setSupportZoom(false); s.setBuiltInZoomControls(false); s.setDisplayZoomControls(false); s.setMediaPlaybackRequiresUserGesture(false); s.setMixedContentMode(WebSettings.MIXED_CONTENT_COMPATIBILITY_MODE); s.setLoadsImagesAutomatically(true); s.setJavaScriptCanOpenWindowsAutomatically(true);
        ...
        webView.setWebViewClient(new WebViewClient(){@Override public void onPageFinished(WebView view,String url){
            ...
            if(url.contains("assets.html")) injectNativeAssets(view);
    ...
    private void injectNativeProjects(WebView view){
        String js="javascript:(()=>{if(typeof AxilortStorage==='undefined')return;const area=document.getElementById('projectArea'),btn=document.getElementById('newProject');if(!area||!btn)return;const esc=s=>String(s).replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',\"'\":'&#39;'}[c]));function render(){let a=[];try{a=JSON.parse(AxilortStorage.list('projects')||'[]').filter(x=>x.folder)}catch(e){}area.innerHTML='';if(!a.length){area.innerHTML='<div class=\"empty\"><div class=\"empty-icon\">□</div><strong>No projects yet</strong><span>Create your first project to see it here.</span></div>';return}a.forEach(x=>{const d=document.createElement('div');d.className='project-bar';d.innerHTML='<div class=\"project-icon\">□</div><div class=\"project-info\"><strong>'+esc(x.name)+'</strong><small>axilort-engine-data / projects / '+esc(x.name)+'</small></div><button class=\"more\" type=\"button\">⋮</button><div class=\"menu\"><button data-a=\"open\">Open Assets</button><button data-a=\"rename\">Rename</button><button class=\"danger\" data-a=\"delete\">Delete</button></div>';const menu=d.querySelector('.menu');d.querySelector('.more').onclick=e=>{e.stopPropagation();document.querySelectorAll('.menu.show').forEach(m=>m.classList.remove('show'));menu.classList.add('show')};d.querySelector('[data-a=open]').onclick=()=>location.href='assets.html?project='+encodeURIComponent(x.name);d.querySelector('[data-a=rename]').onclick=()=>{let n=prompt('New project name',x.name);if(!n)return;n=n.trim();if(!/^[^\\\\/]+$/.test(n))return alert('Invalid project name');if(AxilortStorage.rename('projects/'+x.name,'projects/'+n))render();else alert('Could not rename project')};d.querySelector('[data-a=delete]').onclick=()=>{if(confirm('Delete '+x.name+' and all its files?')){if(AxilortStorage.delete('projects/'+x.name))render();else alert('Could not delete project')}};area.appendChild(d)})}btn.onclick=()=>{let name=prompt('Project name');if(!name)return;name=name.trim();if(!/^[^\\\\/]+$/.test(name)){alert('Invalid project name');return}if(AxilortStorage.createProject(name)){render()}else alert('Could not create project. The name may already exist.')} ;render();})();";view.evaluateJavascript(js,null);
    }

    private void injectNativeAssets(WebView view){
        String js="javascript:(()=>{if(typeof AxilortStorage==='undefined')return;const L=document.getElementById('list'),P=document.getElementById('path'),H=document.getElementById('heading'),Q=document.getElementById('q');if(!L||!P)return;let q=new URLSearchParams(location.search),project=q.get('project')||'',cwd='projects/'+project,shown=[];if(!project||!AxilortStorage.exists(cwd)){location.href='projects.html';return}const esc=s=>String(s).replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',\"'\":'&#39;'}[c]));const size=n=>{n=Number(n)||0;return n<1024?n+' B':n<1048576?(n/1024).toFixed(1)+' KB':n<1073741824?(n/1048576).toFixed(1)+' MB':(n/1073741824).toFixed(1)+' GB'};const valid=n=>{n=String(n||'').trim();return n&&n!=='.'&&n!=='..'&&!n.includes('/')&&!n.includes('\\')};const list=()=>{try{return JSON.parse(AxilortStorage.list(cwd)||'[]')}catch(e){return[]}};function render(){let a=list().filter(x=>!Q.value||x.name.toLowerCase().includes(Q.value.toLowerCase())).sort((a,b)=>Number(b.folder)-Number(a.folder)||a.name.localeCompare(b.name));shown=a;P.textContent='axilort-engine-data/'+cwd;H.textContent=project;L.innerHTML='';if(!a.length){L.innerHTML='<div class=\"empty\"><div class=\"empty-icon\">□</div><strong>'+((Q.value)?'No matching files':'Folder is empty')+'</strong><span>'+((Q.value)?'Try another search.':'Create a folder, file, or upload files.')+'</span></div>';return}a.forEach((x,i)=>{let d=document.createElement('div');d.className='item';d.innerHTML='<div class=\"icon\">'+(x.folder?'DIR':'FILE')+'</div><div class=\"info\"><b>'+esc(x.name)+'</b><small>'+ (x.folder?'Folder':size(x.size))+'</small></div><button class=\"more\" type=\"button\">⋮</button><div class=\"menu\"><button data-a=\"open\">'+(x.folder?'Open':'Read / Edit')+'</button><button data-a=\"rename\">Rename</button><button data-a=\"download\">Download</button><button class=\"danger\" data-a=\"delete\">Delete</button></div>';let m=d.querySelector('.menu');d.querySelector('.more').onclick=e=>{e.stopPropagation();document.querySelectorAll('.menu.show').forEach(z=>z.classList.remove('show'));m.classList.add('show')};d.querySelector('[data-a=open]').onclick=()=>x.folder?(cwd=x.path,render()):edit(x.path);d.querySelector('[data-a=rename]').onclick=()=>rename(x);d.querySelector('[data-a=download]').onclick=()=>download(x);d.querySelector('[data-a=delete]').onclick=()=>{if(confirm('Delete '+x.name+(x.folder?' and everything inside it':'')+'?')&&AxilortStorage.delete(x.path))render()};L.appendChild(d)})}document.addEventListener('click',()=>document.querySelectorAll('.menu.show').forEach(z=>z.classList.remove('show')));function rename(x){let n=prompt('New name',x.name);if(!valid(n)||n.trim()===x.name)return;n=n.trim();if(AxilortStorage.rename(x.path,cwd+'/'+n))render();else alert('Could not rename')}function newFolder(){let n=prompt('Folder name');if(!valid(n))return;if(AxilortStorage.mkdir(cwd+'/'+n.trim()))render();else alert('Could not create folder')}function newFile(){let n=prompt('File name','untitled.txt');if(!valid(n))return;let p=cwd+'/'+n.trim();if(AxilortStorage.exists(p))return alert('File already exists');if(AxilortStorage.writeText(p,'')){render();edit(p)}else alert('Could not create file')}function upload(){let f=document.createElement('input');f.type='file';f.multiple=true;f.onchange=async()=>{for(const x of [...f.files]){let b=await new Promise((r,j)=>{let z=new FileReader();z.onload=()=>r(String(z.result).split(',')[1]||'');z.onerror=j;z.readAsDataURL(x)});AxilortStorage.writeBase64(cwd+'/'+x.name,b)}render()};f.click()}function decode(b){try{let s=atob(b),u=new Uint8Array(s.length);for(let i=0;i<s.length;i++)u[i]=s.charCodeAt(i);return new TextDecoder().decode(u)}catch(e){return null}}function edit(p){let t=decode(AxilortStorage.readBase64(p));if(t===null)return alert('This binary file cannot be edited as text.');let n=prompt('Edit file content',t);if(n!==null&&AxilortStorage.writeText(p,n))render()}function download(x){if(x.folder)return;let b=AxilortStorage.readBase64(x.path);if(!b)return;let s=atob(b),u=new Uint8Array(s.length);for(let i=0;i<s.length;i++)u[i]=s.charCodeAt(i);let a=document.createElement('a');a.href=URL.createObjectURL(new Blob([u]));a.download=x.name;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}window.up=()=>{if(cwd==='projects/'+project){location.href='projects.html';return}let a=cwd.split('/');a.pop();cwd=a.join('/');render()};window.newFolder=newFolder;window.newFile=newFile;window.upload=upload;if(Q)Q.oninput=render;render();})();";view.evaluateJavascript(js,null);
    }
